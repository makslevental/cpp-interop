...
class InterOpLayerWrapper:
    ...
    def get_template(self, scope, name, tmpl_args=[], tpargs=[]):
        if tmpl_args:
            ...
            meth = self._get_template(
                scope, name, ", ".join([a.__name__ for a in tpargs])
            )
...
class TemplateWrapper:
    ...
    def __getitem__(self, *args, **kwds):
        ...
        return gIL.get_template(self._scope, self._name, tmpl_args=args)
    ...
    def __call__(self, *args, **kwds):
        ...
        ol = gIL.get_template(self._scope, self._name, tpargs=[type(a) for a in args])
...
if __name__ == "__main__":
    ...
    CppC = type("C", (), {"handle": gIL.get_scope("C"), "__new__": cpp_allocate})
